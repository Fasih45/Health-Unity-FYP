...
const registerUserasPatient = async (req, res) => {
  try {
    ...
    if (typeof fullName !== "string" || fullName.length < 4) {
      return res
        .status(400)
        .json({ error: "Full name must be at least four characters long" });
    }
    ...
    if (typeof nationality !== "string" || !nationality.trim()) {
      return res
        .status(400)
        .json({ error: "Nationality must be a non-empty string" });
    }
  ...
  try {
    ...
    if (!user.isEmailVerified) {
      ...
      return res.status(401).json({
        error: "Email not verified. Check your email for verification code.",
      });
    }
...
const registerUserasDoctor = async (req, res) => {
  try {
    ...
    if (existingMedicallisence) {
      return res
        .status(400)
        .json({ error: "Medicallisence is already Registered" });
    }
    // Validate password
    if (password.length < 4 || !/[!@#$%^&*(),.?":{}|<>]/.test(password)) {
      return res.status(400).json({
    ...
    if (!medicalLicenseNumber.match(medicalLicenseNumberRegex)) {
      return res.status(400).json({
        error:
          "Medical License Number must be in the format MD12345 or 12345MD",
      });
    }
...
const registerMedicalLab = async (req, res) => {
  try {
    const { username, email, password, labName, labLicense, contactNumber } =
      req.body;
    const existingUsername = await User.findOne({ username });
    const existingUseremail = await User.findOne({ email });
    const { valid, reason, validators } = await emailValidator.validate(email);
    ...
    if (labName.length < 4) {
      return res
        .status(400)
        .json({ error: "Labname must be at least 4 characters long" });
    }
    if (existingLabLicense) {
      return res
        .status(400)
        .json({ error: "Lab license is already registered" });
    }
    ...
    if (!labLicense.match(medicalLicenseNumberRegex)) {
      return res.status(400).json({
        error:
          "Medical License Number must be in the format FPHRA12345 or 12345FPHRA",
      });
    }
    ...
    if (!contactNumber.match(contactNumberRegex)) {
      return res.status(400).json({
        error: "Contact number must be a valid Pakistan phone number",
      });
    }
    ...
    res
      .status(201)
      .json({ message: "Pharmaceutical Lab registered successfully." });
  } catch (error) {
