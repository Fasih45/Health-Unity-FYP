...
const MangeLabTests = () => {
    const [testrec, settestrec] = useState([{ name: "Blood", cost: "300" }, { name: "X-rays", cost: "300" }]);
    ...
    useEffect(() => { setError(""); }, [testrec, Addtestrec]);
    ...
    const addFunction = () => {
        ...
        const duplicates = allTests.filter((item, index) => allTests.some((elem, idx) => elem.name === item.name && idx !== index));
        if (duplicates.length > 0) {
            setError("Test Name Already Exit.");
            setflagerror(true);

        } else if (Addtestrec.every(item => item.name.trim() !== "" && item.cost.trim() !== "")) {
    ...
    const handleSubmit = () => {
        ...
        } else if (testrec.every(item => item.name.trim() !== "" && item.cost.trim() !== "") && Addtestrec.every(item => item.name.trim() !== "" && item.cost.trim() !== "")) {
            console.log('testrec', testrec);
