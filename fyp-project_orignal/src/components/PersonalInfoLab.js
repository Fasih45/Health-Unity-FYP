...
const PersonalInfoLab = (props) => {
  ...
  const handleDaysChange = (selectedOptions) => {
    if (selectedOptions.length === 1 && selectedOptions[0].value === "7 Days") {
      ...
      const days = selectedOptions.filter(
        (option) => option.value !== "7 Days"
      );
  ...
  useEffect(() => {
    if (Apiwrite === "yes") {
      let response = dispatch(registerDoctorProfile("medical_labs", formData));
          ...
          <div className="bg-white dark:bg-gray-700 rounded-md shadow-md p-6">
            ...
            <form onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-5">
                ...
                >
                  ...
                  <input
                    ...
                    className={`form-control border rounded py-2 px-3  ${
                      formErrors.address ? "border-red-500 text-red-500" : ""
                    }`}
                  />
                  {formErrors.address && (
                ...
                <div
                  className={`mb-4 ${
                    formErrors.workingdays ? "border-red-500" : ""
                  }`}
                >
                  <label
                    className="block text-gray-800 font-bold mb-2"
              ...
              {formErrors.test && (
                <p className="text-red-500 text-center text-xs mt-1">
                  Test Name and Cost should not be empty
                </p>
              )}
...
export default PersonalInfoLab;
