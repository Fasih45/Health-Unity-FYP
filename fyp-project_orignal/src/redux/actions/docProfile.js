    ...
    try {
      ...
      if (error.response) {
        ...
        if (status === 404 || status === 422) {
          ...
          dispatch(
            doctorProfileFailure(`Error: ${status} Message:${message}`, status)
          );
        }
      } else {
  ...
  return async (dispatch) => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://localhost:5000/profile/${user}/register`,
        credentials
      );
...
export const rate = (_id, value) => {
  return async (dispatch) => {
    try {
      const response = await axios.post(`http://localhost:5000/profile/rate`, {
        _id,
        value,
      });
      const jsonResponse = response.data;
